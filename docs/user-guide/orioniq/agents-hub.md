...
Click on any invocation to open the **Agent Invocation Details** panel, which contains three tabs plus a toolbar of run-level actions:
...
The toolbar's **Agent definition** and **Agent invocations** links take you from a run to the agent behind it, from any tab: **Agent definition** opens the agent's configuration in a side panel, and **Agent invocations** switches to the Invocations tab filtered to that agent. The reverse path works too — from an agent's configuration form, click **View invocations** to see that agent's run history.

## Provide feedback

Once a run finishes, a **Was this useful?** rating sits in the panel's footer, visible on every tab — it rates the run as a whole, not one tab's content. You can't rate a run that's still in progress.
