...
To stop a run in progress, click **Stop invocation** — from the row's action, or from the **Agent Invocation Details** panel. Stopping can take a few seconds; the row keeps updating until the outcome finalizes as **Stopped**. You can only stop a run from the account where it's running.
...
A run you stopped shows no report here — just a note that the invocation was stopped before it produced an answer.
