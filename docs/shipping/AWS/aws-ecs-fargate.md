...
4. add firelens per container per telelemtry - logs and traces per whatever you need


 a FireLens log configuration to any of your existing task definition. See bottom for logs and traces


For each container you want to collect **logs** from, add the following to its task definition:
...
For each application you want to collect **traces** from, configure the instrumentation to use the centralized OTEL container as an endpoint.
