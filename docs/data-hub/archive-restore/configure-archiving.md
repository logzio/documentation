...
* An AWS S3 bucket with appropriate permissions. The recommended permissions are `PutObject`, `ListBucket`, and `GetObject`. [Learn how to set up your **S3 permissions**.
...
* To set up an IAM role, see
  [_Give AWS access with IAM roles_
* To set up an access key, see
  [_Give AWS access with access keys_
